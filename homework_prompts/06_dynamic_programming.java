...
class Problems {
  ...
  public static int maxConsecutiveSum(int[] arr) {
    ...
    for (int index = 1; index < arr.length; index++) {
      currentMaximum = Math.max(arr[index], arr[index] + currentMaximum);
   ...
   * Problem 2: Bit Flip
   *
   * Given an array of binary values (0 and 1) and N number of flips (convert a 0
   * to a 1), determine the maximum number of consecutive 1's that can be made.
   *
   * Input: An Array of 1's and 0's, and an Integer N denoting the number of flips
   * Output: Integer
   *
   * Example: bitFlip([0,1,1,1,0,1,0,1,0,0], 2) Result: 7
   */
